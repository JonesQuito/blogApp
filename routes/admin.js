...
router.post('/categorias/nova', (req, res) => {
    ...
    if (erros.length > 0) {
        ...
        new Categoria(novaCategoria).save().then(() => {
            ...
            req.flash("erro_msg", "Houve um erro al salvar a categoria, tente novamente!");
...
router.get("/postagens/edit/:id", (req, res) => {
    Postagem.findOne({ _id: req.params.id }).sort({ data: "desc" }).then((postagem) => {
        Categoria.find().sort({ date: 'desc' }).then((categorias) => {

            res.render("admin/editpostagens", { postagem: postagem, categorias: categorias })
        }).catch((err) => {
            req.flash("error_msg", "Erro ao listar categorias");
            res.redirect("admin/postagens/edit/:id");
        })
    }).catch((err) => {
        req.flash("error_msg", "Erro ao recuperar postagem");
        res.redirect("admin/postagens/edit/:id");
    })
